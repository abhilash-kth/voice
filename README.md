...
| **Groq** | `openai/gpt-oss-20b` ⭐ (recommended voice default, sub-second TTFT), `openai/gpt-oss-120b`, `qwen/qwen3.6-27b`, `meta-llama/llama-4-maverick-17b-128e-instruct`, `moonshotai/kimi-k2-instruct`, `minimaxai/minimax-m2.7` | `GROQ_API_KEY=` | Free tier ($0/token, rate-limited per model). Only current models: `llama-3.1-8b`, `llama-3.3-70b`, `qwen/qwen3-32b`, `llama-4-scout` were retired by Groq (Jul–Aug 2026) and are hidden. `openai/gpt-oss-safeguard-20b` is retired from the picker (safety classifier, not a chat model — never use for voice). "Qwen/Qwen3.8-27B" does not exist on Groq — `qwen/qwen3.6-27b` is the real model. |
